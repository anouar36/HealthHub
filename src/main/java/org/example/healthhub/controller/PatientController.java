    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("/updatePatient".equals(pathInfo)) {
            ...
            try {
                ...
                String patientIdStr = req.getParameter("patientId"); // ✅ For admin booking
                ...
                System.out.println("   Patient ID (admin): " + patientIdStr);
                ...
                // ✅ Determine which patient to book for
                if (patientIdStr != null && !patientIdStr.trim().isEmpty()) {
                    // ✅ ADMIN MODE: Book for selected patient
                    patientId = Integer.parseInt(patientIdStr);
                    System.out.println("🔑 ADMIN MODE: Booking for patient ID: " + patientId);
                } else {
                    // ✅ NORMAL MODE: Book for logged-in patient
                    ...
                    System.out.println("✅ Patient: " + patient.getNom() + " (ID: " + patientId + ")");
                }
                ...
                e.printStackTrace();
                resp.sendRedirect(req.getContextPath() +
                        "/user/availabilities?error=invalid-params");
        ...
        // ✅ NEW: Walk-in Appointment (Admin creates appointment with new/existing patient)
        if ("/book-walk-in".equals(pathInfo)) {
            System.out.println("====================================");
            System.out.println("🚶 POST /book-walk-in (Walk-in Appointment)");
            System.out.println("====================================");
            ...
            try {
                ...
                String naissanceStr = req.getParameter("naissance");
                String sexe = req.getParameter("sexe");
                String adresse = req.getParameter("adresse");
                String sang = req.getParameter("sang");
                ...
                if (doctorIdStr == null || date == null || time == null ||
                        cin == null || nom == null || telephone == null) {
                    System.err.println("❌ Missing required parameters");
                    resp.sendRedirect(req.getContextPath() +
                            "/user/availabilities?error=missing-params");
                    return;
                ...
                // ✅ Create PatientCreateDTO
                LocalDate naissance = null;
                if (naissanceStr != null && !naissanceStr.trim().isEmpty()) {
                    naissance = LocalDate.parse(naissanceStr);
                } else {
                    naissance = LocalDate.of(1990, 1, 1); // Default date
                }

                // ✅ Generate default password if patient doesn't exist
                String password = cin + "2024"; // CIN + year

                PatientCreateDTO patientData = new PatientCreateDTO(
                        nom,
                        email != null ? email : cin + "@healthhub.ma", // Default email
                        password,
                        cin,
                        naissance,
                        sexe != null ? sexe : "M",
                ...
                System.err.println("❌ Exception during walk-in booking:");
                e.printStackTrace();
                resp.sendRedirect(req.getContextPath() +
                        "/user/availabilities?error=system-error");
            }
